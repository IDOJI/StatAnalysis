test_MeanDiff_PostHoc = function(data.df, group, variable, alpha, MeanDiff_results.df=results.df){
  ...
  if(test.type=="One-Way ANOVA(Balanced)"){
    ...
    for(i in 1:length(test_names)){
      if(test_names[i]=="SNK"){
        ...
        names(PH.df)[c(1,2,ncol(PH.df))] = c("PostHoc_TestType", "PostHoc_Groups", "p.val")
        rownames(PH.df) = NULL
      }
      PH.list[[i]] = PH.df
    }
