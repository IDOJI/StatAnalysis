test_MeanDiff_PostHoc_games.howell <- function(grp, obs, alpha) {
  ...
  # as it does not assume normality, equal variances or sample sizes.
  # However, the Games-Howell test will often report
  # looser confidence intervals compared to Tukey’s test,
  # and therefore which test to apply should depend on the particular need.
  ...
  colnames(results) <- c('groups', 'Mean Difference', 'Standard Error', 't', 'df', 'p.val', 'upper limit', 'lower limit')
